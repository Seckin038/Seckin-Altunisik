...
export const WhatsappTemplatesSettings: React.FC = () => {
    ...
    const handleSave = async () => {
        if (!editingTemplate.name || !editingTemplate.message) {
            toast.error("Naam en bericht zijn verplicht.");
            return;
        }

        try {
            if (editingTemplate.id) {
                await db.whatsappTemplates.update(editingTemplate.id, editingTemplate);
                toast.success("Sjabloon bijgewerkt!");
            } else {
                await db.whatsappTemplates.add({ ...editingTemplate, id: generateId() } as WhatsappTemplate);
        ...
        return (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
                ...
                <TextareaField
                    ...
                    onChange={e => setEditingTemplate(t => ({...t, message: e.target.value}))}
                    rows={6}
                />
                 <div className="text-xs text-gray-500">
                    Beschikbare variabelen: {`{customer_name}`} {`{stream_label}`} {`{mac_address}`} {`{app_code}`} {`{start_date}`} {`{end_date}`} {`{test_duration}`}
                </div>
    ...
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">WhatsApp Sjablonen</h2>
...
const TextareaField: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }> = ({ label, ...props }) => (
    <div>
        <label htmlFor={props.id || props.name} className="block text-sm font-medium text-gray-700 dark:text-gray-300">{label}</label>
        <textarea {...props} id={props.id || props.name} className="mt-1 block w-full p-2 border rounded-md bg-gray-50 dark:bg-gray-700 dark:border-gray-600" />
    </div>
);